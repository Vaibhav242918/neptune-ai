...
def save_user_to_db(username, email):
    ...
    try:
        c.execute("INSERT INTO users (username, email, sign_in_time) VALUES (?, ?, ?)", (username, email, current_time))
        conn.commit()
    except sqlite3.IntegrityError:
        # User already exists, update sign-in time
        c.execute("UPDATE users SET sign_in_time = ? WHERE username = ?", (current_time, username))
        conn.commit()
    conn.close()
...
# --- SIDEBAR: AUTHENTICATION (SIGN IN / SIGN OUT) ---
st.sidebar.header("🔐 User Authentication")

if not st.session_state['logged_in']:
    auth_mode = st.sidebar.radio("Select Mode", ["Sign In", "Register"])
    ...
    if auth_mode == "Sign In":
        if st.sidebar.button("Sign In"):
            if username_input.strip() != "":
                ...
                save_user_to_db(username_input, email_input if email_input else "N/A")
                st.sidebar.success(f"Welcome back, {username_input}!")
                st.rerun()
            else:
                st.sidebar.error("Please enter a valid username.")
    else:
        if st.sidebar.button("Register & Sign In"):
            if username_input.strip() != "":
                st.session_state['logged_in'] = True
                st.session_state['username'] = username_input
                save_user_to_db(username_input, email_input if email_input else "N/A")
                st.sidebar.success(f"Registered and logged in as {username_input}!")
                st.rerun()
            else:
                st.sidebar.error("Please enter a valid username.")
else:
    ...
    if st.sidebar.button("Sign Out"):
        st.session_state['logged_in'] = False
        st.session_state['username'] = ""
        st.sidebar.info("You have been signed out.")
        st.rerun()

# --- MAIN APPLICATION (Only accessible when signed in) ---
st.title("🌌 Neptune AI - Executive Data Science & ML Hub")

if st.session_state['logged_in']:
    st.markdown(f"Welcome to the platform, **{st.session_state['username']}**! Explore automated EDA, correlation heatmaps, and AutoML benchmarking below.")
    ...
    st.warning("🔒 Please **Sign In** or **Register** from the sidebar to access the Neptune AI platform.")